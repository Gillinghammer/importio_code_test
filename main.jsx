...
var App = React.createClass({
  ...
  fetchData: function() {
      $.get(this.state.searchApi, function(json_data) {
        var data = json_data;
        if (this.isMounted()) {
          this.setState({
      ...
      <div className="container-fluid">
        <div className="row">
          <div className="col-sm-8 col-sm-offset-2">
            <Search handleChange={this.handleChange} val={this.state.searchApi} fetch={this.fetchData} />
          </div>
        </div>
        <div className="row">
          <div className="col-sm-12">
            <Table data={this.state} className="table-responsive" />
          </div>
        </div>
      </div>
      );
